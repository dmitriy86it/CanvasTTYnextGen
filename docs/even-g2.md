...
The code is valid for two minutes. The generic companion discovers one of eight reserved local Bonjour names, then verifies the PIN with SRP-6a before receiving an encrypted connection key issued for that device alone. No address or key is entered manually. Older CT- tokens and eight-digit codes are rejected.
...
- Every handshake receives its own link key. Until the Mac approves the device, that key reaches only the pairing endpoints; a rejected, expired or revoked device's key stops opening any packet, and a key never carries another device's bearer credential.
- A paired device starts read-only. Input, session creation, closing and browser actions each require an explicit toggle on the Mac.
- On a LAN, the API answers only through the encrypted link; plain HTTP `/g2/api/*` requests from other hosts receive 404. HTTPS mode (`publicOrigin`) serves the API directly behind the user's TLS tunnel.
...
Pairing authenticates the short code and separate device grants control API operations. The local transport keys are per device and are destroyed on rejection, expiry or revocation. Keys are long-lived for an approved device, so forward secrecy is not claimed: a device key captured later decrypts that device's recorded traffic. Devices paired with a version that shared one computer-wide key must pair again, since that key is no longer accepted. User profiles, pairing state and keys are excluded from source and package contents.
